...
static INLINE_GL void interpolate(GLVertex *q, GLVertex *p0, GLVertex *p1, float t)
{
...
Practice */
static INLINE_GL int ClipLine1(float denom, float num, float *tmin, float *tmax)
{
    float t;
...
static INLINE_GL void updateTmp(GLContext *c,  GLVertex *q, GLVertex *p0, GLVertex *p1, float t)
{
...
{
    ZB_setTexture(c->zb, c->current_texture->images[0].pixmap,  c->current_texture->images[0].alpha,  c->current_texture->images[0].xsize,  c->current_texture->images[0].ysize);
    ZB_rasterize(c->zb, &p0->zp, &p1->zp, &p2->zp);
}
